...
#[test]
fn test_ivecn_new() {
    let vec: IVecN<5> = IVecN::from_slice([1, 2, 3, 4, 5]);
...
#[test]
fn test_ivecn_dot() {
    let vec1: IVecN<5> = IVecN::from_slice([1, 2, 3, 4, 5]);
    let vec2: IVecN<5> = IVecN::from_slice([6, 7, 8, 9, 10]);
    assert_eq!(vec1.dot(&vec2), 130);

    let vec3: IVecN<6> = IVecN::from_slice([1, 1, 1, 1, 1, 1]);
    assert_eq!(vec3.dot(&vec3), 6);
}

#[test]
fn test_ivecn_length_squared() {
    let vec1: IVecN<6> = IVecN::from_slice([1, 3, 4, -4, 6, -8]);
    assert_eq!(vec1.length_squared(), vec1.dot(&vec1));
}

#[test]
fn test_ivecn_negate() {
    let vec: IVecN<6> = -IVecN::from_slice([1, -2, 3, -4, 5, -6]);
...
fn test_ivecn_add_ivecn() {
    let vec1: IVecN<6> = IVecN::from_slice([1, -2, 3, -4, 5, -6]);
    let vec2: IVecN<6> = IVecN::from_slice([-7, 8, -9, 10, -11, 12]);
...
fn test_ivecn_div_ivecn() {
    let vec1: IVecN<4> = IVecN::from_slice([10, -6, 14, -8]);
    let vec2: IVecN<4> = IVecN::from_slice([-2, 3, -2, 2]);
    let result = vec1 / vec2;
...
fn test_ivecn_mul_scalar() {
    let vec1: IVecN<4> = IVecN::from_slice([5, -6, 7, -8]);
    let result = vec1 * -2;
...
fn test_ivecn_div_scalar() {
    let vec1: IVecN<4> = IVecN::from_slice([10, -6, 4, -8]);
    let result = vec1 / -2;
...
#[test]
fn test_ivecn_index() {
    let vec1: IVecN<6> = IVecN::from_slice([5, -6, 7, -8, 9, -10]);
    assert_eq!(vec1[0], 5);
    assert_eq!(vec1[1], -6);
    assert_eq!(vec1[2], 7);
    assert_eq!(vec1[3], -8);
